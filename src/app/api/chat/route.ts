...
// Soma o texto de todas as partes 'text' de uma UIMessage (v6).
...
const policies = `
  ...
  - Contato: ${legalContent.companyInfo.phone}
...
export async function POST(req: Request): Promise<Response> {
    ...
    const totalChars = messages.reduce((n, m) => n + messageTextLength(m), 0);
        ...
        tools: {
            addToCart: tool({
                ...
                execute: async ({ productId }) => {
                    // O backend apenas registra a intenção; o frontend executa a adição.
                    return { added: true, message: `Produto ${productId} preparado para adição.` };
                },
            }),
        },
    });
