    ...
    return (
        ...
        >
            ...
            <div className="absolute top-0 left-0 h-full w-[70%] max-w-[320px] bg-white shadow-2xl pt-10 pb-6 pl-2 pr-4 flex flex-col overflow-y-auto overscroll-y-contain scroll-smooth safe-top safe-bottom">
                ...
                {/* Navigation */}
                <nav className="space-y-0.5 text-base font-medium text-slate-600 flex-1 w-full mt-4" role="navigation" aria-label="Mobile menu navigation">
                    {mainMenu.map((item, index) => (
                        <button key={index} onClick={() => handleMenuClick(item)} className="flex w-full items-center gap-1.5 hover:text-lyvest-500 py-3 border-b border-slate-50 last:border-0 text-left transition-colors touch-target group">
                            <span className="text-left font-medium text-[15px]">{t(item.translationKey) || item.label}</span>
